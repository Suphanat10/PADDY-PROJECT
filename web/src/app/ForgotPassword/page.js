  ...
  const handleVerifyOtp = async (e) => {
    ...
    setTimeout(() => {
      if (otpValue === "123456") { // Mock Correct OTP
         ...
         setLoading(false);
         setError('รหัส OTP ไม่ถูกต้อง (ลองใช้ 123456)');
      }
    }, 1500);
  };
        ...
        <div className="w-full max-w-2xl">
            ...
            <div className="bg-white p-8 sm:p-10 shadow-xl shadow-emerald-900/5 rounded-2xl border border-gray-100 relative">
                ...
                {step === 1 && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <p className="text-gray-500 mb-8 font-light">กรอกข้อมูลส่วนตัวที่ได้ลงทะเบียนไว้เพื่อทำการตั้งรหัสผ่านใหม่</p>

                        <form onSubmit={handleSendOtp} className="space-y-6 max-w-md mx-auto sm:mx-0">
                            <div>
                                <label htmlFor="email" className="block text-sm font-bold text-gray-700 mb-2">
                                    อีเมล หรือ เบอร์โทรศัพท์มือถือ <span className="text-red-500">*</span>
                                </label>
                                <input
                                    id="email"
                                    name="email"
                            ...
                            <div className="flex items-start gap-3 bg-emerald-50 p-4 rounded-lg border border-emerald-100">
                                ...
                                <div className="text-sm text-emerald-800">
                                    <p className="font-semibold mb-1">หากจำอีเมล หรือ เบอร์โทรศัพท์มือถือไม่ได้</p>
                                    <p>กรุณาติดต่อ Call Center <span className="font-bold">02-016-4888</span></p>
                                </div>
                            </div>
                            ...
                            <div className="pt-4">
                                ...
                                >
                                    {loading ? (
                                        <>
                                            <Loader2 className="animate-spin h-4 w-4" /> กำลังตรวจสอบ...
                                        </>
                                    ) : "ถัดไป"}
                                </button>
                            </div>
                ...
                {step === 2 && (
                    <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                        <div className="text-center mb-8">
                            ...
                            <h2 className="text-2xl font-bold text-gray-900 mb-2">ยืนยันตัวตน</h2>
                            <p className="text-gray-500 font-light">
                                กรอกรหัส OTP ที่ส่งไปยัง <span className="font-medium text-gray-900 bg-gray-100 px-2 py-0.5 rounded">{email}</span>
                            </p>
                        ...
                        <form onSubmit={handleVerifyOtp} className="space-y-8 max-w-md mx-auto">
                            ...
                            <div className="bg-gray-50/80 p-8 rounded-2xl border border-gray-100">
                                ...
                                <div className="text-center mt-6">
                                    ...
                                    <button
                                        type="button"
                                        disabled={timeLeft > 0}
                                        onClick={() => { setTimeLeft(60); setError(''); alert('ส่งรหัส OTP ใหม่แล้ว (Mock)'); }}
                                        className={`text-sm ${timeLeft > 0 ? 'text-gray-400 cursor-not-allowed' : 'text-emerald-600 hover:text-emerald-800 font-semibold'}`}
                                    >
                                        {timeLeft > 0 ? `ขอรหัสใหม่ได้ใน ${timeLeft} วินาที` : "ขอรหัส OTP ใหม่"}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-center gap-4 pt-2">
                                ...
                                >
                                    {loading ? <Loader2 className="animate-spin h-4 w-4" /> : "ยืนยันรหัส OTP"}
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                        ...
                        <form onSubmit={handleResetPassword} className="space-y-6 max-w-md mx-auto">
                            ...
                            <div className="pt-4">
                                ...
                                >
                                    {loading ? <Loader2 className="animate-spin h-4 w-4" /> : "ยืนยันและเปลี่ยนรหัสผ่าน"}
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                {step === 4 && (
                    <div className="text-center animate-in fade-in zoom-in duration-500 py-10">
                        ...
                        <h2 className="text-3xl font-bold text-gray-900 mb-4">เปลี่ยนรหัสผ่านสำเร็จ!</h2>
            ...
            {step !== 4 && (
                <div className="mt-8 text-center">
                    <a href="/auth/login" className="text-gray-500 hover:text-emerald-600 text-sm font-medium flex items-center justify-center gap-2 transition-colors group">
                        <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> ยกเลิกรายการ
                    </a>
                </div>
            )}
        </div>

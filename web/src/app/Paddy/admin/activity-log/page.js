    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={statsData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <YAxis dataKey="name" type="category" width={140} stroke="#475569" fontSize={12} />
                <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }} />
                <Bar dataKey="count" fill="#6366f1" radius={[0, 4, 4, 0]} barSize={24}>
                  {statsData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={['#4f46e5', '#8b5cf6', '#ec4899', '#f43f5e'][index % 4]} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50 text-right">
           <button onClick={onClose} className="px-5 py-2.5 bg-white border border-slate-300 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors shadow-sm">
              ปิดหน้าต่าง
           </button>
        </div>
      </div>
...
const activityData = useMemo(() => {
  ...
  const getActionStyle = (action) => {
    if (action.includes('login') || action.includes('create_account')) return "bg-green-50 text-green-700 border-green-200";
    if (action.includes('create')) return "bg-purple-50 text-purple-700 border-purple-200";
    if (action.includes('update')) return "bg-orange-50 text-orange-700 border-orange-200";
          ...
          <div className="absolute inset-0 bg-white/70 backdrop-blur-sm z-10 flex items-center justify-center">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">ประวัติการใช้งานและกิจกรรมในระบบ</p>
              </div>
                ...
                <div className="lg:col-span-1 bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col justify-center relative overflow-hidden">
                    <div className="relative z-10">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Total Logs</span>
                        </div>
                        <h3 className="text-4xl font-bold text-slate-800">{Data_logs.length.toLocaleString()}</h3>
                        <p className="text-xs text-slate-400 mt-2">รายการบันทึกทั้งหมดในระบบ</p>
                    </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-[calc(100vh-450px)] min-h-[400px]">
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-50 text-sm">
                            {currentLogs.map((log, index) => (
                                <tr key={log.logs_ID} className="hover:bg-indigo-50/30 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-semibold text-slate-700">{log.Account.first_name} {log.Account.last_name}</p>
                                                {/* <p className="text-[10px] text-slate-400 font-mono">ID: {log.user_ID}</p> */}
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-3 py-1 rounded-lg text-xs font-semibold border ${getActionStyle(log.action)}`}>
                ...
                <div className="p-4 border-t border-slate-100 bg-white flex items-center justify-between gap-4">
                    <div className="text-sm text-slate-500">
                        แสดง <span className="font-medium text-slate-700">{indexOfFirstItem + 1}</span> ถึง <span className="font-medium text-slate-700">{Math.min(indexOfLastItem, filteredLogs.length)}</span> จากทั้งหมด <span className="font-medium text-slate-700">{filteredLogs.length}</span> รายการ
                    </div>

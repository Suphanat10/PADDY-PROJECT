...
const handleImpersonate = async (userId) => {
  ...
  : 0}
                      ...
                      >
                        ...
                        <td className="p-6 pr-8 text-right">
                          <div className="flex justify-end gap-2 relative z-10">
                            <button
                              type="button"
                              onClick={() => handleViewFarmSensor(user)}
      ...
      {/* ==================================================================================
          4. FARM SENSOR DATA MODAL 
      ================================================================================== */}
      {isFarmSensorModalOpen && (
        <div className="fixed inset-0 z-[70] overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div
              className="fixed inset-0 bg-[rgba(0,0,0,0.6)] backdrop-blur-sm transition-opacity"
              onClick={() => setIsFarmSensorModalOpen(false)}
            ></div>
            <span className="hidden sm:inline-block sm:align-middle sm:h-screen">
              &#8203;
            </span>

            <div className="relative inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl w-full max-h-[85vh] flex flex-col">
              {/* Header */}
              <div className="bg-gradient-to-r from-emerald-600 to-emerald-700 px-6 py-4 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-3">
                  <Sprout className="text-white" size={24} />
                  <div>
                    <h3 className="text-lg font-bold text-white">
                      ข้อมูลฟาร์มและเซนเซอร์
                    </h3>
                    {selectedUserForFarmSensor && (
                      <p className="text-emerald-100 text-sm">
                        {selectedUserForFarmSensor.first_name} {selectedUserForFarmSensor.last_name}
                      </p>
                    )}
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setIsFarmSensorModalOpen(false)}
                  className="text-emerald-100 hover:text-white p-1 rounded-full hover:bg-emerald-500/30 transition-colors"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
                ...
                ) : farmSensorData.length === 0 ? (
                  <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl">
                    <Tractor className="mx-auto h-12 w-12 text-slate-300 mb-2" />
                    <p className="text-slate-500 font-medium">ไม่พบข้อมูลฟาร์ม</p>
                    <p className="text-slate-400 text-sm mt-1">ผู้ใช้งานนี้ยังไม่มีฟาร์มในระบบ</p>
                  </div>
                ) : (
                  <div className="space-y-6">
                    {farmSensorData.map((farm) => (
                      <div
                        key={farm.farm_id}
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-3 mt-2 text-xs text-slate-500">
                                <span className="flex items-center gap-1">
                                  <LayoutGrid size={12} /> {farm.area} ไร่
                                </span>
                                <span className="flex items-center gap-1">
                                  <Leaf size={12} /> {farm.rice_variety}
                                </span>
                                <span className="flex items-center gap-1">
                                  <Droplets size={12} /> {farm.water_management}
                                </span>
                              </div>
                            </div>
                            <span className="text-[10px] px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100 font-semibold">
                              {farm.planting_method}
                            </span>
                          </div>
                        </div>
                                ...
                                >
                                  ...
                                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-3">
                                    ...
                                    <div className="bg-white rounded-lg p-3 border border-slate-200 text-center">
                                      <div className="text-[10px] text-slate-400 uppercase font-bold mb-1">N (ไนโตรเจน)</div>
                                      <div className="text-lg font-black text-blue-600">
                                        {area.sensor?.n ?? 0}
                                      </div>
                                      <div className="text-[9px] text-slate-400">mg/kg</div>
                                    </div>

                                    {/* P - Phosphorus */}
                                    <div className="bg-white rounded-lg p-3 border border-slate-200 text-center">
                                      <div className="text-[10px] text-slate-400 uppercase font-bold mb-1">P (ฟอสฟอรัส)</div>
                                    ...
                                    {/* K - Potassium */}
                                    <div className="bg-white rounded-lg p-3 border border-slate-200 text-center">
                                      <div className="text-[10px] text-slate-400 uppercase font-bold mb-1">K (โพแทสเซียม)</div>
                                    ...
                                    {/* W - Water Level */}
                                    <div className="bg-white rounded-lg p-3 border border-slate-200 text-center">
                                      <div className="text-[10px] text-slate-400 uppercase font-bold mb-1">W (ระดับน้ำ)</div>
                                      <div className="text-lg font-black text-cyan-600">
                                        {area.sensor?.water_level ?? 0}
                                      </div>
                                      <div className="text-[9px] text-slate-400">cm</div>
                                    </div>

                                    {/* S - Soil Moisture */}
                                    <div className="bg-white rounded-lg p-3 border border-slate-200 text-center">
                                      <div className="text-[10px] text-slate-400 uppercase font-bold mb-1">S (ความชื้นดิน)</div>
                                      <div className="text-lg font-black text-amber-600">
                                        {area.sensor?.moisture ?? 0}
                                      </div>
                                      <div className="text-[9px] text-slate-400">%</div>
                                    </div>

                                    {/* Growth Stage */}
                                    <div className="bg-white rounded-lg p-3 border border-emerald-200 text-center bg-gradient-to-br from-white to-emerald-50">
                                      <div className="text-[10px] text-emerald-600 uppercase font-bold mb-1">ระยะข้าว</div>
                                  ...
                                  {area.measured_at && (
                                    <div className="text-[10px] text-slate-400 text-right">
                                      อัปเดตล่าสุด: {new Date(area.measured_at).toLocaleString("th-TH")}
                                    </div>
                                  )}
                                </div>
                              ))}
              ...
              <div className="bg-slate-100 px-6 py-4 border-t border-slate-200 flex justify-end shrink-0">
                <button
                  type="button"
                  onClick={() => setIsFarmSensorModalOpen(false)}
                  className="px-6 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 transition-colors font-medium"
                >
                  ปิดหน้าต่าง
                </button>
              </div>

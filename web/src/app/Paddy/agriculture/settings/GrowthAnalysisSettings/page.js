...
export default function GrowthAnalysisSettings() {
  ...
  const [config, setConfig] = useState({
    imageCaptureIntervalDays: "3", // growth_analysis_period
    dataIntervalDays: "1",         // data_send_interval_days
    waterMin: 0,
    waterMax: 0,
  });
  ...
  const currentDevice = useMemo(() => {
    return devices.find(
      (d) => d.device_registrations_ID == selectedRegID
    );
  ...
  // เปลี่ยน farm → เลือก device แรก
  useEffect(() => {
    if (selectedFarm) {
      const first = devices.find(d => d.farm_name === selectedFarm);
      if (first) setSelectedRegID(first.device_registrations_ID);
    }
  ...
  useEffect(() => {
    if (currentDevice?.setting) {
      setConfig({
        imageCaptureIntervalDays: String(
          currentDevice.setting.growth_analysis_period ?? "3"
        ),
        dataIntervalDays: String(
          currentDevice.setting.data_send_interval_days ?? "1"
        ),
        waterMin: currentDevice.setting.Water_level_min ?? 0,
        waterMax: currentDevice.setting.Water_level_mxm ?? 0,
      });
    }
  }, [currentDevice]);

  // คำนวณเวลาถัดไป (วัน → ms)
  useEffect(() => {
    const days = parseFloat(config.imageCaptureIntervalDays);
    if (days > 0) {
      const next = new Date(
  ...
  }, [config.imageCaptureIntervalDays, currentTime]);
  ...
  const handleSave = () => {
    GrowthSettings(
      ...
      setSuccessMessage,
      setConfig,
      config.imageCaptureIntervalDays,
      config.dataIntervalDays
    );
  };
  ...
  const getTimeRemainingText = () => {
    if (config.imageCaptureIntervalDays === "0") return "หยุด";
    const days = parseFloat(config.imageCaptureIntervalDays);
    // แปลงวันเป็นชั่วโมงเพื่อแสดงผลให้ดูง่ายถ้าต่ำกว่า 1 วัน
    const totalHours = days * 24;
    
    if (totalHours < 24) {
       return `รอ ${Math.round(totalHours)} ชม.`;
    }
    return `รอ ${Math.floor(days)} วัน`;
  };
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative flex-1">
                    <select
                      ...
                      value={config.dataIntervalDays}
                      onChange={(e) =>
                        setConfig({
                          ...config,
                          dataIntervalDays: e.target.value,
                        })
                      }
                    >
                      ...
                      <option value="30">ทุกๆ 30 วัน</option>
                    </select>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative flex-1">
                    <select
                      ...
                      value={config.imageCaptureIntervalDays}
                      onChange={(e) =>
                        setConfig({
                          ...config,
                          imageCaptureIntervalDays: e.target.value,
                        })
                      }
                    >
                      {/* ใช้ค่าชุดเดียวกับด้านบนหรือจะกำหนดแยกก็ได้ แต่นี่ใช้หน่วยเป็นวันตามปกติของการวิเคราะห์ภาพ */}
                      <option value="1">ทุกๆ 1 วัน</option>
                      <option value="3">ทุกๆ 3 วัน</option>
                      <option value="7">ทุกๆ 7 วัน</option>
                      <option value="10">ทุกๆ 10 วัน</option>
                ...
                <div className="flex items-center justify-between mb-6 relative z-10">
                  ...
                  <span
                    className={`text-[10px] font-semibold px-2.5 py-1 rounded-full border ${
                      config.imageCaptureIntervalDays !== "0"
